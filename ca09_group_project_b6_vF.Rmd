...
Using the predict funtion we can get the same results.
```{r}
testdata <- c(114, 4.86, 2, 3, 3.96, "Downtown","Private room","Private room in residential home")
testnames <- c("number_of_reviews", "review_scores_rating", "bedrooms", "accommodates", "reviews_per_month","neighbourhood_simplified","room_type","prop_type_simplified")
test <- rbind(testdata)
colnames(test) <- testnames
testframe <- data.frame(test)

testframe <- testframe %>% 
  mutate(number_of_reviews = parse_number(number_of_reviews),
         review_scores_rating = parse_number(review_scores_rating),
         bedrooms = parse_number(bedrooms),
         accommodates = parse_number(accommodates),
         reviews_per_month = parse_number(reviews_per_month)) 
predict(model8_1, testframe, interval = "confidence")
```
...
In conclusion, the model derived is able to explain the price of Airbnb's in San Francisco quite well, given the high adjusted R-squared of 0.59. We are also able to provide nuances to the city's hotel / airbnb pricing, as we observe differences in price across neighborhoods and property and room types. 
